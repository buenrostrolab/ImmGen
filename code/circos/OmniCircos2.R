...
## main ##
circos2 <- function (mapping=mapping, xc=400, yc=400, R=400, W=W,  
    ...
    if (cir.m == 0){
      ...
      for (chr.i in c(1:chr.num)){
        ...
        if (print.chr.lab){
          ...
          draw.text.rt(xc, yc, r+r.j, w.m, chr.t, cex=3);
        } 
        if (scale){
  ...
  if (type == "immDensity"){
    ...
    ## BUEN COLOR
    #bc <- c('#25032E', '#670253', '#A30B48', '#DE1515', '#FF8534', '#FFE272', '#EEFFAB', '#F2FFEA')
    bc <- c('#1873CC', '#1798E5', '#00BFFF', '#4AC596', '#00CC00', '#A2E700', '#FFFF00', '#FFD200', '#FFA500')
    rbPal  <- colorRampPalette(bc)  ## EDIT COLORS HERE
    col.i  <- rbPal(100)[cut(dat.c, breaks = 100)];
    col.m  <- matrix(col.i, ncol=num.col, nrow=num.row);
    ...
    for (chr.i in 1:chr.num){
      ...
      for (i in 1:nrow(dat)){   
        ...
        w.to   <- scale.v(as.numeric(dat[i,2]), v1, v2, v3, v4);
          r.to <- r.from - w.i;
          if (v.wide > 0.5){
            draw.arc.pg(xc, yc, w.po, w.po+v.wide, r.from, r.to, col=col.c[i], border=NULL);
          } else {
            draw.line(xc,  yc, w.po, r.from, r.to, col=col.c[i], lwd=lwd1);
          }
          r.from <- r.to;
...
draw.text.rt <- function(xc, yc, r, w, n, col="black", cex=1, side="out"){
  ...
  text(x, y, adj=0, offset=1, labels=n, srt=0, 
